...
import { MessageCircle, Send, Loader2, Globe, Users, ChevronDown } from 'lucide-react'
...
export function NewsWallMulticanal({ 
  ...
  const currentFlashes = activeTab === 'publico' ? publicFlashes : (memberFlashes || [])
  ...
  return (
    <div className='space-y-6'>
      <div className='flex items-center gap-2 bg-white/[0.02] border border-white/5 rounded-2xl p-2 w-fit'>
        <button
        ...
        {currentFlashes.length === 0 ? (
          ...
          >
            <p className='text-white/40 text-sm'>
              {activeTab === 'publico' 
                ? 'No hay noticias publicas disponibles.' 
                : 'No hay noticias para miembros.'}
            </p>
          </motion.div>
        ) : (
            ...
            {currentFlashes.map((flash) => (
              ...
              >
                ...
                <p className='text-white/80 leading-relaxed whitespace-pre-wrap mb-6'>
                  {activeTab === 'publico' ? flash.texto_publico : flash.texto_miembros}
                </p>
