...
export function MemberManagementTable({ members, commissions, currentUserRole = 'miembro' }: Props) {
  const isAdmin = currentUserRole === 'admin'
  const [loadingId, setLoadingId] = useState<string | null>(null)
  const [activeTab, setActiveTab] = useState<'pending' | 'active' | 'all'>(
    currentUserRole === 'admin' ? 'pending' : 'active'
  )
  ...
  return (
    <div className="space-y-8">
      {/* Aprobación Manual por Email (Solo para Admins) */}
      {isAdmin && (
      ...
      <div className="space-y-6">
        {/* Selector de Pestañas (Solo para Admins) */}
        {isAdmin && (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[var(--text-secondary)] text-[10px] uppercase tracking-widest border-b border-[var(--border-subtle)]">
                  ...
                  {isAdmin && <th className="py-4 px-4 font-medium text-right">Acción</th>}
                </tr>
              </thead>
              <tbody className="text-sm">
                {filteredMembers.map((m) => {
                  const currentCommission = m.commission_members?.[0]?.commissions?.id || ''
                  const commissionName = commissions.find(c => c.id === currentCommission)?.name || 'Sin asignar'
                    ...
                    <tr key={m.id} className="border-b border-[var(--border-subtle)] hover:bg-white/5 transition-colors group">
                      ...
                      <td className="py-4 px-4">
                        {isAdmin ? (
                          ...
                          <span className="text-[var(--text-secondary)] text-xs truncate max-w-[140px] block">
                            {commissionName}
                          </span>
                        )}
                      </td>
