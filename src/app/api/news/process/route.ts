...
export async function POST(request: NextRequest) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.log(`[API /news/process] IA completada en ${elapsed}s (intento ${attempt}/${MAX_ATTEMPTS})`)
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1)
      lastError = err
      console.error(`[API /news/process] Error en intento ${attempt}/${MAX_ATTEMPTS} después de ${elapsed}s:`, err.message)

      if (attempt < MAX_ATTEMPTS) {
        console.log(`[API /news/process] Reintentando en ${RETRY_DELAY_MS}ms...`)
  ...
  console.error(`[API /news/process] Error definitivo después de ${elapsed}s y ${MAX_ATTEMPTS} intentos`)
